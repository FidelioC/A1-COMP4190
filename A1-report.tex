...
(b)\;& x^* : \nabla f(x^*) = 0 \\
    ...
    & I x^* = A^{-1} b\\
    & x^* = A^{-1} b\\
(c)\;& \text{From the function $f(x)$ we know that the curve of the graph is upward} \\
     & \text{since $x^T A x > 0$ for all non-zero $x$} \hfill .. (1) \\[2pt]
     & \text{Furthermore, $A$ is also symmetric as given in the question} \hfill .. (2) \\[2pt]
     & \text{Based on (1) and (2), we can infer that $det(A) > 0$, thus $A$ is invertible, thus, it has a unique solution} \hfill .. (3) \\[2pt]
     & \text{From part (b), we also have calculated the value of $x^*$ for the matrix $A$} \hfill .. (4) \\[2pt]
     & \text{Thus, we can conclude that the solution is unique and minimum based on (3) and (4)}
\end{align*}
...
(e)\;& x_{k+1} = x_k - \alpha 2(x-2) \\
    ...
    & \\
    & \text{To make, } (1-2\alpha)^k \rightarrow 0, \text{we need to make } |(1-2\alpha)| < 1 \\
    & \text{Solve for, } |(1-2\alpha)| < 1\\
    & -1 < 1-2\alpha < 1\\
    & -1-1 < 1-2-1\alpha < 1-1\\
    & -2 < -2\alpha < 0\\
    & 0 < \alpha < 1 \\
    & \text{Thus, the condition of } \alpha \text{ under which it converges to } x^* \text{ is }0 < \alpha < 1
\end{align*}
